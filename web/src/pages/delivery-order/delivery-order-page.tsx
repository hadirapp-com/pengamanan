...
import { useDeliveryOrderColumns } from "./delivery-order-columns";
import { Unlock, RefreshCw, Calendar, Download } from "lucide-react";
...
// Enable sample data mode for development/testing
const USE_SAMPLE_DATA = false; // Set to true to use sample data instead of API
...
export default function DeliveryOrderPage() {
  ...
  const [exportFormat, setExportFormat] = useState<"xlsx" | "xlsm">("xlsx");
  ...
  const {
    ...
    },
    {
      ...
      enabled: !USE_SAMPLE_DATA, // Disable when using sample data
  ...
  const transformedData: LotData[] = useMemo(() => {
    ...
    return lotData.result.map((item: Record<string, unknown>) => {
      const prepStatus = item.preparation_status || "open";
      const delStatus = item.delivery_status || "open";
      const isInProgress =
        prepStatus === "in progress" || delStatus === "in progress";
      ...
      return {
        ...
        deliveryYear:
          item.delivery_year?.toString() || new Date().getFullYear().toString(),
        partNumber: item.part_number,
        partName: item.part_name,
        poNumber: item.po_number,
        kdLotNoTo: item.kd_lot_no_to,
        ...
        status: isInProgress
          ? "in progress"
          : prepStatus === "closed" && delStatus === "closed"
            ? "completed"
            : "not started",
    ...
    if (customer && customerData?.result) {
      ...
      if (customerInfo) {
        return transformedData.filter(
          (lot) => lot.customerId === customerInfo.id,
        );
      }
    }
  ...
  const columns = useDeliveryOrderColumns({
    onUnlock: (lot) => {
      setSelectedLot(lot);
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        (error instanceof Error ? error.message : "Failed to unlock lot");
  ...
  const handleExport = async () => {
    const selectedData = Object.keys(rowSelection)
      .map((idx) => {
        const item = filteredData[parseInt(idx)];
        if (!item) return null;
        return {
          lot: item.lot,
    ...
    if (selectedData.length === 0) {
      toast.error("Pilih minimal satu lot untuk di-export");
      return;
    }
    ...
    try {
      const response = await axiosInstance({
        method: "POST",
        url: deliveryEndpoint.export,
        data: { lots: selectedData, format: exportFormat },
        responseType: "blob",
      });
      ...
      link.setAttribute(
        "download",
        `PRONES_Shipment_Export_${format(new Date(), "yyyy-MM-dd")}.${exportFormat}`,
      );
      ...
      toast.success(`Berhasil meng-export ${selectedData.length} lot`);
      setRowSelection({}); // Clear selection after export
    } catch (error: unknown) {
      ...
      <div className="mb-4 flex justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1">
            <Select value={exportFormat} onValueChange={(value: "xlsx" | "xlsm") => setExportFormat(value)}>
            ...
            <Button
              ...
              variant="default"
              size="sm"
              disabled={isExporting || Object.keys(rowSelection).length === 0}
            >
              {isExporting ? (
                <>
      ...
      <Dialog open={unlockDialogOpen} onOpenChange={setUnlockDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to unlock lot{" "}
              <strong>{selectedLot?.lot}</strong>?
              <br />
              <br />
              This will allow users to change lots. Current progress will be
              lost.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
